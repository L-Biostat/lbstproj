...
#'
#' @param overwrite Logical; if `TRUE`, overwrite existing report files in the
#'   `results/reports/` directory. If `FALSE`, a unique name will be generated
#'   to avoid overwriting, by appending a counter to the filename (e.g. `_001`,
#'   `_002`, etc.). Default is `FALSE`.
#'
#' @return The path to the moved and renamed report file, invisibly.
#'
#' @export
run_report <- function(overwrite = FALSE) {
  ...
  return(invisible(moved_path))
}

#' Moves a report to a different location and rename it uniquely
#'
#' Move the report file generated by `run_report()` from the `reports/`
#' directory to the `results/reports/` directory, renaming it to include:
#' - the current date
...
#'
#' @param old_path The current path of the report file to be moved (with
...
#' Build a report file name with date, version, and optional counter and ext
#'
#' @param extension The file extension for the report (e.g., "html", "pdf").
#' @param counter An optional counter to append to the file name for uniqueness.
#'   If `NULL`, no counter is added.
